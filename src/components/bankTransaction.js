...
const onSave = async (voucherData, drTotal, crTotal) => {
   ...
   // const Data = {
    ...
    return (
     ...
     <label style={{paddingRight: '200px'}}>Bank Selection :
     <select onChange={(e) => handleChangeBank(e)}>
       {bankData.map((items) => (
         <option value={items.id} required> (Bank ID-{items.bankID}) (Bank Name-{items.bankName}) (Bank Account No.-{items.bankAcctNo})</option>
      ))}
     </select>
 ...
 Bank G/L No. :
  ...
  />
   ...
   <div className="select-container" >
   <label style={{paddingRight: '195px'}}>G/L Selection:
     ...
     {glData.map((item) => (
       <option value={item.id} required> (G/L No-{item.glNo}) (G/L Sub No-{item.glSub}) (Department-{item.department}) (G/L Name-{item.glName})</option>
        ...
        <input
         ...
         <br />
         <td><button style={buttonStyle} type="button" onClick={onAddVoucher}>Add Voucher </button></td>
