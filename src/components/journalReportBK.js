...
onSearch (e) {
  // e.preventDefault();
  sDate = this.state.startDate; // El.current.value
  eDate = this.state.endDate; // El.current.value
   if (sDate === '' ) {
...
//  })
  ...
  endDate = moment(eDate).format("DD-MM-YYYY");
                   Axios.get(url+`/api/VoucherSearch`,
                    {
                     params: {
                         companyID: companyID,
                         startDate: moment(sDate).format("YYYY-MM-D"),
      ...
      if (data.length>0) {
           ...
           // console.log(this.state.customer);
             ...
             alert("No voucher transaction between "+startDate+" and "+endDate) 
          } // if
               })
