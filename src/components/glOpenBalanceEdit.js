...
//   var sDate = '';
 // alert(glNo+' = '+glSub);
  ...
  class glOpenBalanceEdit extends React.Component {
    constructor(props) {
      super(props);
   ...
   this.glAmountEl = React.createRef();
   this.opBalanceEl = React.createRef();
   this.editedBalanceEl = React.createRef();
...
glData = res.data;
if (glData.length === 0) {
  window.location.href ='glOpenBalance';
}
// alert(glData[0].glDescription);
 ...
 this.setState({ depNo: glData[0].department });
   ...
   this.glAmountEl.current.value = glData[0].glAmount.toFixed(2);
   this.opBalanceEl.current.value = glData[0].opBalance.toFixed(2);
  // window.alert(glData[0].opBalance);
   this.editedOpBalance.current.value ='0.00';
...
//alert("server connection Fail, please retry");
...
onValueChange = (event) => {
  ...
  handleBack() {
    this.props.history.push("/glOpenBalance");
  };
      ...
      return (
        ...
        <form style={mystyle} onSubmit={this.handleSubmit}>
          <fieldset>
           ...
           <h1>Edit General Ledger Opening Balance</h1>
           <label>G/L No. :
          ...
          <label> G/L Name :
          <input type="text" maxLength={50} ref={this.glNameEl} name="glName" readOnly= {true} required />
          </label>

          <label>G/L Description :
          <input type="text" maxLength={50} ref={this.glDescriptionEl}  name="glDescription" readOnly= {true} required />
          </label>
          ...
          <select value={this.state.gType} onChange={this.handleChangeType}>
            {typeData.map((item) => (
              <option ref={this.glTypeEl} value={item.glType} required disabled="disabled"> {item.glType} {item.glTypeName}</option>
           ))}
          </select>
          ...
          <select value={this.state.depNo} onChange={this.handleChangeDep}>
            {depData.map((item) => (
              <option ref={this.departmentEl} value={item.department} required disabled="disabled" > {item.department} {item.description} </option>
           ))}
          </select>
          ...
          <label>G/L Current Amount :
          <input type="number" maxLength={13} ref={this.glAmountEl}  name="glAmount" readOnly= {true} required />
          </label>
          <label>G/L Opening Balance :
          <input type="number" maxLength={13} ref={this.opBalanceEl}  name="opBalance" readOnly= {true} required />
          </label>
...
export default glOpenBalanceEdit;
