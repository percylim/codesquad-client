...
console.log('Final dara to be saved:'.data);
Axios
  ...
  .then(res => {
   if (res.data.length === 0) {
    ...
    alert('Error load G/L List Info : ' + err.message);
   return false;
  });
...
// alert(res.data);
 if (res.data === 'Existed') {
    ...
    console.error('❌ Axios error:', err);
    alert('Error load Voucher Verify : ' + err.message);
   return false;
  });

Axios
.get(`/api/purchaseInvoiceVerify`,
  {
   params: {
           companyID: companyID,
           supplierID: supplierID,
...
// alert(res.data);
 if (res.data === 'Existed') {
    ...
    console.error('❌ Axios error:', err);
    alert('Error Purchase Invoice Verify : ' + err.message);
   return false;
  });
...
Axios.post(`/api/purchaseInvoice`,data)
...
.then(res => {
  ...
  //  alert(text);
    ...
    console.error('❌ Axios error:', err);
    alert('Error Save Purchase Invoice : ' + err.message);
   return false;
  });
...
Axios.post(`/api/purchaseVoucher`, voucherData
...
.then(res => {
  ...
  //  alert(text);
    ...
    console.error('❌ Axios error:', err);
    alert('Error Save Purchase Voucher : ' + err.message);
   return false;
  });
         ...
         //  setDueDate(format(date, "dd/MM/yyyy"));
          ...
          Axios
            ...
            .then(res => {
             ...
             console.error('❌ Axios error:', err);
             alert('Error load Customer Info : ' + err.message);
              return false;
        });
     ...
     const formatInputPrice = async (e) => {
      ...
      //  alert(itemTax);
       ...
       if (invType === 'PUR') {
        ...
        invoiceTotal+=Number(itemTotal);
        invoiceDiscountTotal+=Number(itemDiscount);
        invoiceNetTotal+=Number(itemNetTotal);
 //alert(itemTotal+ ' = '+itemNetTotal);
       } else {
...
<br />
<label style={{ paddingLeft: "10px"}}>
<a style={{ marginRight: '.6rem' }}>Tax :</a>
...
<div />
 ...
 <p></p>
<td></td>
<td><button className = "fa fa-book" style={{backgroundColor: "blue", color: "white", width: '200px', marginLeft: '20rem'}} onClick={() => onNew() }>New Invoice </button></td>
<td><button className = "fa fa-home" style={{backgroundColor: "grey", color: "black", width: '100px'}} onClick={() => handleHome()}>Home</button></td>
<td><button className = "fa fa-plus" style={buttonStyle} type="button" onClick={onAddPurchaseItem}>Add Purchase Item </button> </td>
 ...
 <a style={{ marginLeft: '1rem', marginRight: '1.1rem' }}>Voucher No. :</a>
    ...
    <Tooltip
      ...
      >  
          ...
          >
         <i class="btn btn-primary fa fa-download" style={{height: '40px'}}></i>   
        Get Next Voucher No
          </button>
...
<p></p>
<p></p>
           <td><button style={{backgroundColor: "green", color: "white", width: '200px', marginLeft: '7rem'}} onClick={() => onPrint(voucherData, totalDrAmt, totalCrAmt)}>
           <i class='fa fa-print' style={{marginRight: '5px'}}></i>
           Print Voucher</button></td>
           <td><button style={{backgroundColor: "red", color: "white", width: '400px'}} onClick={() => onSave(voucherData, totalDrAmt, totalCrAmt)}>
           <i class='fa fa-save' style={{marginRight: '5px'}}></i>
           Save Invoice and Voucher</button></td>
           <td><button style={{backgroundColor: "cyan", color: "black", width: '300px'}} onClick={() => onAddVoucher()}>
           <i class='fa fa-plus' style={{marginRight: '5px'}}></i>
           Add Voucher Item</button></td>
