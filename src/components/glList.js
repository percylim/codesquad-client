...
// let input = this.state.input;
  ...
  const onLoadGlInfo = () => {
    if (data.length > 0) {
      alert('G/L Account already with data, cannot load sample data');
      return false;
    }
 Axios.get(`${url}/api/loadGlInfo`)
  .then((res) => {
    let dataInfo = res.data;
    if (dataInfo.length > 0) {
      const updated = dataInfo.map((row) => ({
        ...row,
        companyID,
      }));

      setData(updated); // async update
      isDisabled = true;

      // ✅ use updated here, not data
    //  alert('data to save: ' + updated.length);

      // save immediately
      fetch(url + '/api/glSave', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updated), // ✅ use updated
      })
        ...
        //  alert(text);
          if (text === 'Success') {
            isDisabled = true;
          //  alert('Sample G/L Account Data Successfully saved');
          } else {
            isDisabled = false;
           // alert('Fail to Save sample G/L Account Data');
          }
        });
    } else {
      alert('No sample G/L Account data to load');
      isDisabled = false;
    }
  })
  .catch((err) => {
    console.error('Error loading GL Info:', err);
    isDisabled = false;
  });
...
if (data.length > 0) {
  ...
  return (
    ...
    <div className="col-sm-12 btn btn-success" style={{width: '1560px'}}>
    General Ledger Listing
    </div>
   <div> 
  <button 
    className="fa fa-plus-square" 
    style={{paddingLeft: '0px', backgroundColor: 'blue', color: 'white', height: '40px', width: '200px', padding: '1px', marginLeft: '1rem'}} 
    onClick={() => onhandleNew()}
  >
    Add New G/L
  </button>
  ...
  <button 
    ...
    style={{paddingLeft: '10px', backgroundColor: 'yellow', color: 'black', height: '40px', width: '300px', padding: '1px', marginLeft: '1rem'}} 
  ...
  <CSVLink style={{ paddingLeft: '1400px', color: 'black'}} {...csvReport}>
    Export to CSV
  </CSVLink>
</div>
</div>
