    ...
    // 
      BSData.push(newDatOEQ3);    
          ...
          const user = {  companyID: companyID, year: year};  
            ...
            Axios
            .post(url + '/api/balanceSheetDelete', user  )
            .then(res => {
            }, []);
  
        Axios
        .post(url + '/api/balanceSheetUpdate', BSData
        ).then(res => {
          if (res.data === 'Success') {
...
<button
        ...
        type='button'
        class='btn btn-danger float-right fa fa-eraser'
        onClick={() => onClear()}
