...
import generatePDF from "./reportGeneratorBK";
...
function PurchaseInvoicePayment() {
  ...
  const toMySQLDate = (dateStr) => {
      ...
      return moment(dateStr, ['DD/MM/YYYY', 'YYYY-MM-DD', moment.ISO_8601]).format('YYYY-MM-DD');
    };
  // const [custData, setCustData] = useState([]);
 ...
 // const [barcode, setBarcode] = useState("");
  ...
  const formattedDate = toMySQLDate(txnDate);
...
//      setGlID(glData[i].id);
  ...
  const onPrint = async (voucherData, drTotal, crTotal) => {
   ...
   // generatePDF(newDatas, 'PV')
     generatePDF(newDatas, headers, 'PV.pdf')
...
//return false
  ...
  const onSearchInvoice = async () => {
    ...
    Axios
      .get(`/api/purchaseInvoiceSummary`,
        {
          params: {
            companyID: companyID,
            supplierID: supplierID,
...
const onSearchVoucher = async () => {
  ...
  const jvInit = `PV${yearSuffix}${monthPadded}`;  // "JV2605" – matches DB column

  try {
    const res = await Axios.get('/api/lastVoucherNo', {
      params: { companyID, jvInit }
    });

    let nextVoucherNo;
    if (res.data && res.data.length > 0) {
      const lastVoucher = res.data[0].voucherNo;   // e.g., "JV2605-2" or "PV2605-13"
      const parts = lastVoucher.split('-');
      const prefix = parts[0];                     // "JV2605" or "PV2605"
      const suffix = parseInt(parts[1], 10);       // 2 or 13
      const newSuffix = suffix + 1;                // 3 or 14

      // Ensure the prefix is "PV" for payment vouchers
      let newPrefix = prefix;
      if (prefix.startsWith('JV')) {
        newPrefix = 'PV' + prefix.slice(2);        // "PV2605"
      }
      nextVoucherNo = `${newPrefix}-${newSuffix}`; // "PV2605-14"
    } else {
      // No existing voucher → start at 1
      nextVoucherNo = `PV${yearSuffix}${monthPadded}-1`;
    }
...
//alert(barcode);
        ...
        <table class="table" style={{ paddingTop: '1px', border: '1px solid black' }}>
          ...
          <tbody style={mystyle} >
            {data.map(item => {
             ...
             <td></td>
             <td></td>
               <td style={{ textAlign: "center", backgroundColor: "cyan" }}>Total :</td>
 ...
 <div >
        ...
        <div style={{ flex: 1, height: '2px', backgroundColor: 'transparent' }} />
...
/>
        ...
        <p></p>
        <td></td>
        <td><button className = "fa fa-file" style={{ backgroundColor: "blue", color: "white", width: '200px' }} onClick={() => onNew()}>New Payment </button></td>
        <td><button className = "fa fa-home" style={{ backgroundColor: "grey", color: "black", width: '100px' }} onClick={() => handleHome()}>Home</button></td>
        <td><button className = "fa fa-print" style={{ backgroundColor: "green", color: "white", width: '300px' }} onClick={() => onPrintPayment(data)}>Print Payment Detail</button></td>
        <td><button className = "fa fa-save" style={buttonStyle} type="button" onClick={onAddPayment}>Update Payment</button> </td>
        ...
        <div>
          ...
          <label style={{ paddingLeft: '10px' }}>
            ...
            <td><button style={{ backgroundColor: "green", color: "white", width: '200px' }} onClick={() => onPrint(voucherData, totalDrAmt, totalCrAmt)}>
            ...
            Save Payment and Voucher</button></td>
