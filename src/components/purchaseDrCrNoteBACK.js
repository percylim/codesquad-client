        ...
        .get(`/api/glList`,
          {
           params: {
              ...
              //console.log("fetch glData:"+res.data);
...
const onSearchInvoice = async (e) => {
    ...
    // Build prefix that matches database values (e.g., 'JV2605')
       ...
       const jvInit = `JV${yearSuffix}${monthPadded}`;     // 'JV2605'
       ...
       try {
           ...
           if (res.data && res.data.length > 0) {
               const lastVoucherNo = res.data[0].voucherNo; // e.g., "JV2605-2"
               // Split on the dash
               const parts = lastVoucherNo.split('-');
               const prefix = parts[0];                     // "JV2605"
               const suffix = parseInt(parts[1], 10);       // 2
               if (isNaN(suffix)) {
                   throw new Error('Invalid suffix in voucher number');
               }
               const newSuffix = suffix + 1;                // 3
               nextVoucherNo = `${prefix}-${newSuffix}`;    // "JV2605-3"
           } else {
               nextVoucherNo = `${jvInit}-1`;               // "JV2605-1"
           }
   
           setVoucherNo(nextVoucherNo);
         // alert(`Next Voucher No.:`+voucherNo);
       } catch (err) {
           console.error('Error fetching last voucher:', err);
           alert('Error loading last voucher number: ' + err.message);
       }
   };
...
<td><button className = "fa-fa-home" style={{backgroundColor: "grey", color: "black", height: '50px',width: '100px', marginLeft: '.5rem'}} onClick={() => handleHome()}>Home</button></td>
<td><button className = "fa fa-book" style={{backgroundColor: "blue", color: "white", height: '50px',width: '150px'}} onClick={() => onNew() }>New Note </button></td>
<td><button className = "fa fa-file" style={{backgroundColor: "green", color: "white", height: '50px',width: '200px'}} onClick={() => handleAddNote()}>Add Dr/Cr Note</button></td>
...
<p></p>
 ...
 <div style={{ marginTop: '10px', textAlign: 'center' }}>
  <button style={{backgroundColor: "green", color: "white", width: '200px', marginRight: '10px'}} onClick={() => onPrint(voucherData, totalDrAmt, totalCrAmt)}>
    <i className='fa fa-print' style={{marginRight: '5px'}}></i> Print Voucher
  </button>
  <button style={{backgroundColor: "red", color: "white", width: '350px', marginRight: '10px'}} onClick={() => onSave(voucherData, totalDrAmt, totalCrAmt)}>
    <i className='fa fa-save' style={{marginRight: '5px'}}></i> Save Invoice and Voucher
  </button>
  <button style={{backgroundColor: "yellow", color: "black", width: '250px'}} onClick={() => onAddVoucher()}>
