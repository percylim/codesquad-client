...
const onSearchNoteNo = async () => {
  ...
  .then(res => {
    ...
    // 情况2：有 docNum 字段
    if (res.data.docNum !== undefined && res.data.docNum !== null) {
      lastNumber = res.data.docNum;
      console.log("Using docNum:", lastNumber);
    }
    // 情况3：有 documentNo 字段，从中提取序号
    else if (res.data.documentNo) {
      const match = res.data.documentNo.match(/-(\d+)$/);
      if (match && match[1]) {
        lastNumber = parseInt(match[1]);
        console.log("Extracted from documentNo:", lastNumber);
      }
    }
    // 情况4：返回的是数组格式
    else if (Array.isArray(res.data) && res.data.length > 0) {
      const lastRecord = res.data[0];
      if (lastRecord.docNum) {
        lastNumber = lastRecord.docNum;
      } else if (lastRecord.documentNo) {
        const match = lastRecord.documentNo.match(/-(\d+)$/);
    ...
    const vNo = `${invType}${cYear}-${newNumber}`;
    console.log(`Last number: ${lastNumber}, New number: ${newNumber}, Document No: ${vNo}`);
...
//const VOUCHER_TYPE = 'PNV';  // Purchase Voucher
...
<td></td>
<td><button className = "fa fa-print" style={{backgroundColor: "green", color: "white", width: '300px', marginLeft: '.5rem'}} onClick={() => onPrintNote()}>Print Credit/Debit Note</button></td>
<td><button className = "fa fa-home"style={{backgroundColor: "grey", color: "black", width: '100px', marginLeft: '.5rem'}} onClick={() => handleHome()}>Home</button></td>
<td><button className = "fa fa-book" style={{backgroundColor: "blue", color: "white", width: '150px'}} onClick={() => onNew() }>New Note </button></td>
<td><button className = "fa fa-plus" style={{backgroundColor: "cyan", color: "black", width: '200px'}} onClick={() => handleAddNote()}>Add Dr/Cr Note</button></td>
...
<p></p>
           ...
           <td><button className = "fa fa-print" style={{backgroundColor: "green", color: "white", width: '200px'}} onClick={() => onPrint(voucherData, totalDrAmt, totalCrAmt)}>Print Voucher</button></td>
           <td><button className = "fa fa-save" style={{backgroundColor: "red", color: "white", width: '300px'}} onClick={() => onSave(voucherData, totalDrAmt, totalCrAmt)}>Save Invoice and Voucher</button></td>
           <td><button className = "fa fa-plus" style={{backgroundColor: "cyan", color: "black", width: '250px'}} onClick={() => onAddVoucher()}>Add Voucher Item</button></td>
