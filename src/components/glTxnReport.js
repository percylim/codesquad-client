...
const GlTxnReport = () => {
  ...
  useEffect(() => {
    Axios.get(`${url}/api/glTypeInfo`, { params: { companyID } })
      .then(res => {
  ...
  useEffect(() => {
    if (glType) {
      Axios.get(`${url}/api/glSelectList`, { params: { companyID, gType: glType } })
        .then(res => {
          setGlData(res.data);
          if (res.data.length > 0) {
  ...
  return (
    <div className="container" style={{ 'height': "50px", 'margin': '0px'}} >
     ...
     <div class="col-sm-10 btn btn-info" style={{ 'color': 'black', marginTop: '0px', width: '1600px'}}>
