   ...
   handleChangeImg(e) {
    ...
    // setImage('http://localhost:9005/fetchImage/'+imgID);
     ...
     // alert(imgID);
      ...
      let img = 'http://localhost:9005/fetchImage/'+imgID;
      ...
      if(this.validate()){
       ...
       // alert(this.sexEl.current.value);
        ...
        const user= {
          ...
          categoryID: catID,
          productImage: EscapeStr(this.productImageEl.current.value),
          imageID: imgID,
         };
         //var name1 =  EscapeStr(user.companyName);
       // alert(Level);
        fetch('http://localhost:9005/productNew', {
          method: 'POST',
       ...
       this.unitPriceEl.current.value = 0.00;
  ...
  //  };
   ...
   const subStyle = { 
    ...
    //const options=list.slice(0,5-userLevel);
     ...
     Axios({
      method: 'post',
      url: 'http://localhost:9005/categoryInfo',
      data: body
    })
    .then(res => {
      console.log(res);
      data = res.data; 
      catID = data[0].description;
      // window.alert(data[1].description);
    });
    Axios({
      method: 'post',
      url: 'http://localhost:9005/imageInfo',
      data: body
    })
    .then(res => {
      console.log(res);
   ...
   //  const {img} = this.state;
      ...
      return (
        ...
        <form style={mystyle}  onSubmit={this.handleSubmit}>
          ...
          onBlur={ this.formatInput } maxLength={15} placeholder="0.00" ref={this.unitPriceEl}   name="unipPrice"  />
          </label>
          ...
          <select onChange={this.handleChangeImg}>
            {imgData.map((item) => (
